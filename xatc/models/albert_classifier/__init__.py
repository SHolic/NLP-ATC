...
from ...common.loader import AlbertDatasets
from .model import AlbertClassificationModel
...
def train(data_path,
    ...
    # load training data
    albert_datasets = AlbertDatasets(path=data_path, pre_trained_path=pre_trained_model_path)
    train_data, test_data = albert_datasets.load(n_jobs=n_jobs, train_size=train_size, batch_size=batch_size)

    total_steps = len(train_data) * epochs

    # init the albert model
    model = AlbertClassificationModel(num_labels=albert_datasets.num_labels,
                                      finetune=finetune,
    ...
    if model_path is not None:
        _save_model(path=model_path, model=model, pre_trained_model_path=pre_trained_model_path,
                    max_length=albert_datasets.max_length, label2index=albert_datasets.label2idx)
...
def predict(data=None, data_path=None, model_path=None, **kwargs):
    ...
    albert_datasets = AlbertDatasets(path=data_path, data=data, pre_trained_path=pre_trained_model_path)
...
class Predictor:
    ...
    def predict(self, data=None, data_path=None):
        albert_datasets = AlbertDatasets(path=data_path, data=data, pre_trained_path=self.pre_trained_model_path)
